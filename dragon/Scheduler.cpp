...
void Schedule::init(void) 
{
	...
	//Set up timer2. 1ms per interrupt
...
/////////////////////////////////////////////////////////////////
// uint8_t Schedule::editTask(uint8_t taskIndex, void (*pFn)(), uint16_t del, uint16_t per, boolean co_op)
// Modify a task
//
//     taskIndex     -   ID of the task to be edited
//     other parameters are same as addTask	
// Return:  
//     RETURN_ERROR  -   Can't edit the task.
//     RETURN_NORMAL -   Succeed in editting the task
//////////////////////////////////////////////////////////////////
uint8_t Schedule::editTask(uint8_t taskIndex, void (*pFn)(), uint16_t del, uint16_t per, boolean co_op)
{
	boolean returnCode;
	...
	{
		// There is no task
		g_errorCode = ERROR_CANNOT_EDIT_TASK;
		returnCode = RETURN_ERROR;  // Return an error code 
	}
	else
	{
		g_Tasks[taskIndex].pTask = pFn;

		g_Tasks[taskIndex].delay  = del;
		g_Tasks[taskIndex].period = per;

		g_Tasks[taskIndex].co_op = co_op;
